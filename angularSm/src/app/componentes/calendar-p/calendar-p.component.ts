...
import { Evento } from 'src/app/modelos/evento.modelo';
...
@Component({
  ...
  styleUrls: ['./calendar-p.component.scss'],
  providers: [UsuarioService,SalaService,ReunionService,TipoSalaService,EventoService]
})

export class CalendarPComponent implements OnInit, AfterViewInit{
...
*/
  ...
  constructor(
    ...
    this._eventoService.obtenerEventos().subscribe ( reuniones => {
  ...
  obtenerEventos(){
    this._eventoService.obtenerEventos().subscribe(
      response => {
         this.datos = response.eventosEncontrados;
